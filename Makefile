...
pkgdatadir = $(datadir)/mastercore
pkgincludedir = $(includedir)/mastercore
pkglibdir = $(libdir)/mastercore
pkglibexecdir = $(libexecdir)/mastercore
...
CONFIG_HEADER = $(top_builddir)/src/config/master-config.h
CONFIG_CLEAN_FILES = libmasterconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libmasterconsensus.pc.in \
	...
	$(top_srcdir)/src/config/master-config.h.in \
...
ACLOCAL = ${SHELL} /root/master/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/master/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/master/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/master/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/master/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
MINIUPNPC_LIBS = 
...
OTOOL64 = 
PACKAGE = mastercore
PACKAGE_BUGREPORT = https://github.com/masterpay/master/issues
PACKAGE_NAME = master Core
PACKAGE_STRING = master Core 1.2.0
PACKAGE_TARNAME = mastercore
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/master
abs_srcdir = /root/master
abs_top_builddir = /root/master
abs_top_srcdir = /root/master
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/master/build-aux/install-sh
...
pkgconfig_DATA = libmasterconsensus.pc
BITCOIND_BIN = $(top_builddir)/src/masterd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/master-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/master-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
OSX_APP = master-Qt.app
OSX_DMG = master-Core.dmg
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_master_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_master_coverage.info test_master.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/master-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/master-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/master-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/master-config.h
$(top_srcdir)/src/config/master-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/master-config.h src/config/stamp-h1
libmasterconsensus.pc: $(top_builddir)/config.status $(srcdir)/libmasterconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/master-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/master-Qt

$(OSX_DMG): $(APP_DIST_EXTRAS)
	$(GENISOIMAGE) -no-cache-inodes -D -l -probe -V "master-Core" -no-pad -r -dir-mode 0755 -apple -o $@ dist
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/master-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_master.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_master -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_master_filtered.info: test_master.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_master_filtered.info
#	$(MKDIR_P) qa/tmp
...
#rpc_test.info: test_master_filtered.info
...
#test_master_coverage.info: baseline_filtered_combined.info test_master_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_master_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_master_filtered.info block_test_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_master_filtered.info -a block_test_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_master.coverage/.dirstamp:  test_master_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_master.coverage/.dirstamp total.coverage/.dirstamp

# master: disable the test until upstream switches from convoluted comparison tool
...
clean-local:
	rm -rf coverage_percent.txt test_master.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
