...
const char * const BITCOIN_CONF_FILENAME = "master.conf";
const char * const BITCOIN_PID_FILENAME = "masterd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "master" is a composite category enabling all master-related debug output
            if(ptrCategory->count(string("master"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\masterCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\masterCore
    // Mac: ~/Library/Application Support/masterCore
    // Unix: ~/.mastercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "masterCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/masterCore";
#else
    // Unix
    return pathRet / ".mastercore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty master.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override master.conf
